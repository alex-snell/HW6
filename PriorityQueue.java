 ...
 * comparable operator. But, this class allows for more sophisticated priority
 * comparisons (e.g., see below on aging discussions as we also discussed in
 * class).
 *
 * To allow the application (aka, user of this class) to have very basic
 * priority management, the class does provide a method, changePriority(), such
 * that you can change a priority of an element already on the queue. The
 * invoker will need to know the node's element to use this (aka, have a handle
 * to it), which is returned from the method which adds the element to the
 * queue. This is for very basic priority management. Additionally, the invoker
 * can dequeue an element no matter where it is on the queue before it gets
 * processes (popped off), and later, if desired, enqueue it with a higher
 * and/or lower priority.
 *
 * A more sophisticated approach would be to implement a weighted priority
 * scheme that optionally allows aging and/or fair scheduling techniques to
 * ensure there is no starvation. This is simple to do with this class. It is
 * accomplished, by specifying an object type for the priority that supports
 * this capability, and has defined an appropriate comparable operator.
 *
 * Public Methods: ---------------
 *
 * int size() - Returns the number of elements in the queue boolean isEmpty() -
 * Returns true if this collection contains no elements. void clear() - Removes
 * all of the elements from this priority queue. Node peek() - Retrieves, but
 * does not remove, the head of this queue, or returns null if this queue is
 * empty. Node add(E,P) - Inserts the specified element into this priority
 * queue. Node offer(E,P) - Inserts the specified element into this priority
 * queue. Node remove() - Retrieves and removes the head of this queue. This
 * method differs from poll only in that it throws an exception if this queue is
 * empty. Node poll() - Retrieves and removes the head of this queue, or returns
 * null if this queue is empty. boolean contains(E) - Returns true if element
 * 'E' is in queue, else false. ArrayList<E> toArray - returns an array
 * consisting of the element values in the queue. void printPriorityQueue - This
 * method is for debugging purposes, it will print out the min heap
 *
 *
 * Additional methods: The type 'Node' is returned from several methods. It acts
 * as a handle to the queued object when it is on the priority queue. The
 * following methods are available on the object 'Node' when the object is still
 * on the queue: - changePriority - given a handle to a node, you can change its
 * priority - remove - given a handle to a node, you can remove it from the
 * queue directly
 *
 */
class PriorityQueue<E, P> {
     ...
     *
     * @return: Node - An element of type Node is returned, else null if queue
     * is empty.
     */
    public Node peek() {
        if (size() == 0) {
     ...
     * lowest level of the tree. It then will pull up the inserted element
     * towards the root until it is in its correct location on the heap.
     *
     * The parameters are the object representing the element to queue, along
     * with its priority object. The method returns the type Node, which allows
     * a handle to the inserted item by the invoking application (if desired).
     * The invoking application has the availability to invoke any method
     * available on type Node after it is returned, including changing the
     ...
     * @return: Node - Returns an object of type 'Node' representing the newly
     * inserted element
     */
     ...
     *
     * @return: Node - Returns an object of type 'Node' representing the removed
     * element
     */
    public Node remove() {
        if (tree.size() == 0) {
     ...
     *
     * This method is called when enqueuing an element on the queue. The shape
     * of the min heap is expected to be correct, and this is being called to
     * re-heapify the heap by pulling up the value in the logical tree based on
     * the element value at the location specified by the array index location
     ...
     *
     * @return: ArrayList<E> - an array consisting of the element values in the
     * queue
     */
     ...
     * iterating through the array. The tree structure is overlayed on the
     * array.
     */
     ...
     * implementation of the min heap is an array of type Node. The constructor
     * of Node is based on a generic interface and takes the object types of 'E'
     * and 'P' from the instantiation of the class PriorityQueue which
     * encapsulates this object.
     *
     * Object type 'E' is the object type of the value of each element in the
     * min heap. And the type 'P' is the data type of its priority.
     *
     * Several key method are provided on the Node object. The object is
     * returned to the application on methods of PriorityQueue which add it to
     * the minHeap. That returned object represents a handle to that element
     * which is currently in the queue (min heap).
     *
     ...
     * remove - remove the element from the queue. - isvalid - returns true if
     * handle to an element is still in the queue, or returns false if it was
     * removed already.
     */
    public class Node {
